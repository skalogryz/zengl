...
uses
  ...
  zgl_fx,
  zgl_textures,
  zgl_textures_png, // ¬ажный момент, об€зательно один раз подключить модуль с поддержкой нужного формата данных
  zgl_render_target,
  zgl_sprite_2d,
  zgl_font,
  zgl_text,
...
begin
  ...
  // с теми, что указаны в tex_CreateZero.  аждому таргету также указан флаг RT_FULL_SCREEN, отвечающий за то, что бы
...
procedure Draw;
  var
    i : Integer;
begin
  // ƒл€ показательности разницы в скорости будем рендерить 9 раз
  for i := 0 to 9 do
    begin
      ...
      case Mode of
        0:
          begin
            // Simple-режим отличаетс€ от остальных следующим:
            // - дл€ сохранени€ содержимого, нужно перерисовывать текстуру саму в себ€,
            //   без использовани€ блендинга
            // - т.к. этот способ все рисует в сцену, а потом в копирует в текстуру, то
            //   после этого на экране все остаетс€. ћожно чистить через scr_Clear
