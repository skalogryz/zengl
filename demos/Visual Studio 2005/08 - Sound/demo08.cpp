...
void Init()
{
	...
	icon[ 0 ] = tex_LoadFromFile( "../data/audio-stop.png" );
	icon[ 1 ] = tex_LoadFromFile( "../data/audio-play.png" );
...
void Draw()
{
	...
	ssprite2d_Draw( icon[ state ], ( SCREEN_WIDTH - 128 ) / 2.0f, ( SCREEN_HEIGHT - 128 ) / 2.0f, 128.0f, 128.0f, 0 );
	...
	if ( col2d_PointInRect( (float)mouse_X(), (float)mouse_Y(), r ) )
	{
		...
		ssprite2d_Draw( icon[ state ], ( SCREEN_WIDTH - 132 ) / 2.0f, ( SCREEN_HEIGHT - 132 ) / 2.0f, 132.0f, 132.0f, 0, 155 );
...
void Proc()
{
	...
	if ( mouse_Click( M_BLEFT ) )
	{
		...
		if ( col2d_PointInRect( (float)mouse_X(), (float)mouse_Y(), r ) && ( audio == 0 ) )
			audio = snd_PlayFile( "../data/music.ogg" );
	}
