...
void Init()
{
  texTux = tex_LoadFromFile( "../data/tux_stand.png", 0xFF000000, TEX_DEFAULT_2D );
  ...
  rtarget = rtarget_Add( tex_CreateZero( 512, 512, 0x000000, TEX_DEFAULT_2D ), RT_FULL_SCREEN );
}
...
void Draw()
{
  ...
  // RU: Рисуем в него
  // EN: Render to it.
  asprite2d_Draw( texTux, (float)( rand() % ( 800 - 64 ) ), (float)( rand() % ( 600 - 64 ) ), 64.0f, 64.0f, 0, rand() % 9 + 1, 255, FX_BLEND );
  ...
  ssprite2d_Draw( rtarget->Surface, ( 800 - 512 ) / 2.0f, ( 600 - 512 ) / 2.0f, 512.0f, 512.0f, 0, 255, FX_BLEND );
