...
void Init()
{
	...
	texBack = tex_LoadFromFile( "../data/back01.jpg" );

	texGround = tex_LoadFromFile( "../data/ground.png" );
	// RU: Указываем размер кадра в текстуре.
	// EN: Set the size of single frame for texture.
	tex_SetFrameSize( &texGround, 32, 32 );

	texTuxWalk = tex_LoadFromFile( "../data/tux_walking.png" );
	tex_SetFrameSize( &texTuxWalk, 64, 64 );
	texTuxStand = tex_LoadFromFile( "../data/tux_stand.png" );
